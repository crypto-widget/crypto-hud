...
<p align="center">
  轻量、本地、安全的 Windows 与 macOS 桌面行情 HUD。
</p>
...
<p align="center">
  ...
  <img alt="平台：Windows 与 macOS" src="https://img.shields.io/badge/platform-Windows%20%7C%20macOS-0078d4">
...
Crypto HUD 是一个使用 Rust 和 Slint 构建的原生 Windows 与 macOS 桌面应用，目前处于 alpha 阶段。
Windows 是当前已经验证的平台；macOS 代码和打包流程已经准备好，仍需首次 Mac 真机验收。

- 作为单个原生桌面进程运行。
- 使用真正的桌面窗口，而不是 WebView 或浏览器宿主 UI。
- 已包含主界面、托盘控制、全局隐藏/显示快捷键、本地持久化、插件加载和原生打包脚本。
- 默认快捷键：Windows 使用 `Alt+C`，macOS 使用 `Option+C`。
...
- Windows 使用 `Alt+C`、macOS 使用 `Option+C` 隐藏或显示所有组件。
...
Crypto HUD 目前使用本地发布脚本，而不是 GitHub Actions 自动发布流程。Windows 打包已经过本地验证；
macOS 打包必须在 Mac 上执行，并完成 [MACOS.md](MACOS.md) 中的真机检查清单。
macOS CI 会构建并烟测 ARM64、Intel 和 Universal 产物，但不会发布版本，也不会读取签名凭据。
...
在 macOS 上，通过 `mise` 管理的工具链构建当前架构的 `.app` 和 DMG：
...
macOS 脚本还支持 `--arch universal` 通用二进制、Developer ID 签名和 Apple 公证。
环境准备、完整命令、产物和待执行的真机测试矩阵见 [MACOS.md](MACOS.md)。
